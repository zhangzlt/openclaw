...
async def main():
    state = {"token": None, "login_count": 0}
    ...
    async with async_playwright() as p:
        ...
        for sel in ['input[placeholder*="itcode"]', 'input[placeholder*="请输入itcode"]', 'input[type="text"]']:
            try:
        ...
        for i in range(15):
            ...
            print(f"  ... waiting ({i*3}s, logins={state['login_count']})")
        ...
        if not state["token"]:
            ...
            if not state["token"]:
                try:
                    ...
                    print(f"  [debug] page.content() error: {e}")
                    try:
                    ...
                    try:
                        ...
                        print(f"  [debug] all cookies: {json.dumps([{'name': c['name'], 'domain': c.get('domain','')} for c in cookies])}")
                    except Exception as e2:
                        print(f"  [debug] debug read error: {e2}")
                    pass
        ...
        if state["token"]:
            ...
            print(f"  Cookies: {json.dumps([{'name': c['name'], 'domain': c.get('domain','')} for c in cookies])}")
            await page.screenshot(path='screenshots/login_debug.png')
            await browser.close()
            return None
